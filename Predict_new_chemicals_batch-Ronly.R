...
tv.desc.names <- names(tv.train[[1]])[-(1:2)]
...
for (j in 1:100) {
  ...
  if (newchem.smiles !="") {
    ...
    mol.desc.clean <- mol.desc[,names(mol.desc) %in% c("smiles",tv.desc.names)]
    ...
    if (!nocarbon & !hasmetals) {
      ...
      for (tval in ToxValuesNames) {
        ...
        # scale with fixing zero variance values
        all.desc.scale <- as.data.frame(apply(all.desc, 2, function(y) (y - mean(y)) / sd(y) ^ as.logical(sd(y))))
        x.new.scale <- all.desc.scale[1,]
        tv.desc.scale <- all.desc.scale[-1,]
        # Calculate z-score
        trainAD <- GET_NN_STATS(tv.desc.scale, K = 1, maxchunk = 0, dist_type ="Euclidean2")
        Zscores <- GET_AD(x.new.scale, tv.desc.scale, K = 1, NN_STATS = trainAD) 
        y.pred[tval,"appl.domain"]<-Zscores[1,1]
      }
    }
